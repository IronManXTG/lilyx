...
from FallenMusic import SUDO_USERS, app, Ass
from FallenMusic.Helpers.Tasks import add_task, rm_task
...
async def iter_edit(message: Message, text: str):
    async for m in Ass.iter_history(message.chat.id):
...
async def executor(client, message: Message):
    ...
    if len(message.command) < 2:
        return await eor(message, text="**ɢɪᴠᴇ sᴏᴍᴇ ᴛᴇxᴛ sᴏ ɪ ᴡɪʟʟ ᴛʀʏ ᴛᴏ ᴇxᴇᴄᴜᴛᴇ ɪᴛ.**")
    ...
    if not status.from_user:
        status = await Ass.get_messages(status.chat.id, status.message_id)
...
async def shellrunner(_, message: Message):
    if len(message.command) < 2:
        return await eor(message, text="**ᴇxᴀᴍᴩʟᴇ :**\n/sh git pull")
